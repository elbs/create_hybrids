...
uint64_t upper_mask = (~lower_mask);// & ((1ull<<W)-1);
...
uint64_t extract_number (void) {
	if (mt_index >= N) {
		if (mt_index > N) assert (0);
	}
	twist ();
	uint64_t y = MT[mt_index];
	y = y ^ ((y >> U) & D);
	y = y ^ ((y << S) & B);
	y = y ^ ((y << T) & C);
	y = y ^ (y >> L);
...
map<unsigned long long int, int> patterns, distinct_regions;
int npatterns = 0;
...
double rand01 (void) {
	return (myrand () % 1000000000) / 999999999.0;
}

int c1 = 17, c2 = 14, c3 = 2, pc_bits = 7;

struct probabalistic_counter {
	int	x;

	probabalistic_counter (void) {
		reset ();
	}
	...
	void maybe_increment (void) {
		if (rand01() <= (1.0 / pow (base, x))) x++;
	}

	void maybe_decrement (void) {
		if (x > 1) if (rand01() <= (1.0 / pow (base, x))) x--;
	}

	probabalistic_counter operator ++ () {
		probabalistic_counter c;
		c.x = this->x;
		maybe_increment ();
		return c;
	}

	probabalistic_counter operator ++ (int) {
		maybe_increment ();
		return *this;
	}
	...
	void halve (void) {
		if (this->x > c1) this->x -= c2; else this->x /= c3;
	}

	int value (void) {
		return (int) pow (base, x);
	}
};
...
int 
	use_pcount = 1,		// use probabalistic counters
	blocks_per_region = 21,	// blocks per region  // Elba
	real_depth = 3,	// depth to search in the graph including traversing edges with outdegree 1
	dequeue_per_cycle = 4,	// maximum number of prefetches to dequeue and issue per cycle
	inc_late = 2,		// increment edge count by this much on a late prefetch
	inc_useful = 5, 	// increment edge count by this much on a useful prefetch
	dec_useless = 2, 	// decrement edge count by this much on a useless prefetch
	counter_width = 7,	// counter width
	depth = 2,		// maximum depth to search the CFG, not including edges with outdegree 1 // Elba 
...
node compress (uint64_t x) {
	...
	// removing this for now because it screws up a limit study
...
#define CFG_LG_SETS	12

#define CFG_ASSOC	(1<<CFG_LG_ASSOC)
#define	CFG_SETS	(1<<CFG_LG_SETS)

// this is a maximum value; the real value is set in main so ignore this initialization
...
void countup (cfg_edge *b) {
	...
	if (use_pcount) {
		if (b->pcount.x >= ((1<<pc_bits)-1))
			for (int i=0; i<CFG_SETS; i++) for (int j=0; j<CFG_ASSOC; j++) CFG[i][j].pcount.halve();
		b->pcount++;
	} else {
...
cfg_edge *insert_cfg (uint64_t source, uint64_t target) {
	...
	int minr = 0;
	if (use_pcount) {
		for (r=0; r<CFG_ASSOC; r++) if (S[r].pcount.value() < S[minr].pcount.value()) minr = r;
	} else {
...
void access_cache (
	...
	if (S[r].prefetched) {
		...
		if (S[r].edge) {
			if (use_pcount) {
				for (int j=0; j<dec_useless; j++) S[r].edge->pcount--;
			} else {
...
void depth_first_search (
	...
	if (d <= depth && real_d <= real_depth) {
		...
		int total = 0;
		if (use_pcount) {
			for (auto p=L.begin(); p!=L.end(); p++) total += (*p)->pcount.value();
		} else {
		...
		for (auto p=L.begin(); p!=L.end(); p++) {
			...
			double myprob;
			if (use_pcount) {
				myprob = piprod * ((*p)->pcount.value() / (double) total); 
			} else {
				myprob = piprod * ((*p)->count / (double) total); 
			}
			if (myprob >= mp[d])
...
list<prefetch_info> demand_fetch (uint64_t fetch_addr) {
	...
	if (region != last_region) {
		if (last_region != INVALID_REGION) {
			...
			{
				cfg_edge *b = current_edge;
				unsigned long long int x = 0;
				for (int i=0; i<blocks_per_region; i++) {
					if (b->spatial_pattern[i]) x |= (1<<i);
				}
				patterns[x]++;
				npatterns++;
			}
...
void O3_CPU::l1i_prefetcher_initialize() {
	seed_mt (0xdeadbeef);
	char *s;
	int cfg_lg_sets = 5;                        // Elba
	s = getenv ("CFG_LG_SETS"); if (s) { 
		sscanf (s, "%d", &cfg_lg_sets); 
	}
	cfg_sets = 1 << cfg_lg_sets;
	printf ("cfg_sets=%d\n", cfg_sets); 
	strcpy (benchmark, "unknown");
	s = getenv ("BMARK"); if (s) { strcpy (benchmark, s); }
	s = getenv ("MP0"); if (s) { sscanf (s, "%lf", &mp[0]); printf ("mp0=%g\n", mp[0]); }
	mp[1] = mp[0];
	mp[2] = mp[0];
	mp[3] = mp[0];
	mp[4] = mp[0];
	s = getenv ("PC_BITS"); if (s) { sscanf (s, "%d", &pc_bits); printf ("pc_bits=%d\n", pc_bits); }
	s = getenv ("USE_PCOUNT"); if (s) { sscanf (s, "%d", &use_pcount); printf ("use_pcount=%d\n", use_pcount); }
	s = getenv ("BASE"); if (s) { sscanf (s, "%lf", &base); printf ("BASE=%g\n", base); }
	s = getenv ("MP1"); if (s) { sscanf (s, "%lf", &mp[1]); printf ("mp1=%g\n", mp[1]); }
	s = getenv ("MP2"); if (s) { sscanf (s, "%lf", &mp[2]); printf ("mp2=%g\n", mp[2]); }
	s = getenv ("MP3"); if (s) { sscanf (s, "%lf", &mp[3]); printf ("mp3=%g\n", mp[3]); }
	s = getenv ("MP4"); if (s) { sscanf (s, "%lf", &mp[4]); printf ("mp4=%g\n", mp[4]); }
	s = getenv ("DEPTH"); if (s) { sscanf (s, "%d", &depth); printf ("depth=%d\n", depth); real_depth = depth + 1; }
	s = getenv ("WBNLIM"); if (s) { sscanf (s, "%d", &would_be_nice_limit); printf ("would be nice limit = %d\n", would_be_nice_limit); }
	s = getenv ("BPR"); if (s) { sscanf (s, "%d", &blocks_per_region); printf ("blocks per region = %d\n", blocks_per_region); }
	s = getenv ("INC_LATE"); if (s) { sscanf (s, "%d", &inc_late); printf ("inc_late = %d\n", inc_late); }
	s = getenv ("INC_USEFUL"); if (s) { sscanf (s, "%d", &inc_useful); printf ("inc_useful = %d\n", inc_useful); }
	s = getenv ("DEC_USELESS"); if (s) { sscanf (s, "%d", &dec_useless); printf ("dec_useless = %d\n", dec_useless); }
...
void O3_CPU::l1i_prefetcher_cycle_operate() {
	...
	for (int i=0; i<dequeue_per_cycle; i++) {
		...
		if (L1I.get_occupancy(3, 0) < L1I.get_size(3, 0)) {
			...
			} else if ((L1I.get_occupancy(3, 0) == 0) && would_be_nice_queue.size()) {
