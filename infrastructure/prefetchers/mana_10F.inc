...
// PARAMETERS
uint64_t MANA_TABLE_SINGLE_SETS = 1 * 1024;             // Elba: E34 CHANGED: 4096 --> 1024
uint64_t MANA_TABLE_SINGLE_WAYS = 6;                    // Elba: D34 CHANGED: 4 --> 6
...
bool MANA_SUPPORT_MULTIPLE_TABLES = false;              // Elba: G46 CHANGED: true --> false
...
bool USE_HOBPT = true;
uint64_t HOBPT_sets = 64;                               // Elba: E36 CHANGED: 128 --> 64
uint64_t HOBPT_ways = 8;
bool HOBPT_uses_LRU = true;

// the regions size shows number of blocks encoded in the footprint
const int regionSize = 10;                              // Elba: C45 CHANGED: 8 --> 10

// the number of spatial regions that are prefetched upon MANA's activation
int theLookahead = 6;                                   // Elba: C41 CHANGED: 3 --> 6

// the length of each stream
int theTrackerSize = 12;                                // Elba: C42/D38 CHANGED: 5 --> 12

// the length of SRQ
int theSRQSize = 24;                                    // Elba: C43/D37 CHANGED: 8 --> 24

// the number of streams (SABs)
int theStreamCount = 4;                                 // Elba: G41 CHANGED: 1 --> 4
